...
export default function AppSidebar({ closeSidebar }: AppSidebar) {
  ...
  const offsetPosition =
    screen.width > 1280 ? -(screen.width / 4) : -screen.width;
  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={{
          ease: "easeOut",
          duration: animationPosition < 0 ? 0.1 : 0.2,
        }}
        initial={{ x: offsetPosition }}
        animate={{ x: animationPosition }}
        className={
          "bg-slate-50 h-screen flex flex-col p-4 drop-shadow-lg md:w-1/5"
        }
      >
        <div className={"flex p-2 items-center"}>
          ...
          <button
            ...
            title={
              loca.localization.constructionPage.sidebar.backBtnHint[
                loca.language
              ]
            }
          >
        ...
        <div className={"p-4 flex flex-col gap-8 flex-grow"}>
          <ul className={"drop-shadow-md"}>
            <SidebarButton
              icon={<TbLayersDifference className={"w-full h-full"} />}
              href={`/app/${router.query.user}/${router.query.id}/construction`}
            >
              {
                loca.localization.constructionPage.sidebar.quizType[
                  loca.language
                ]
              }
            </SidebarButton>

            {activeTemplate?.type === "multipleCategories" ? (
              <SidebarButton
                icon={<MdDashboardCustomize className={"w-full h-full"} />}
                href={`/app/${router.query.user}/${router.query.id}/construction/category`}
              >
                {
                  loca.localization.constructionPage.sidebar.category[
                    loca.language
                  ]
                }
              </SidebarButton>
            ...
            >
              {
                loca.localization.constructionPage.sidebar.questions[
                  loca.language
                ]
              }
            </SidebarButton>
            <SidebarButton
              ...
              href={`/app/${router.query.user}/${router.query.id}/construction/design`}
            >
              {loca.localization.constructionPage.sidebar.design[loca.language]}
            </SidebarButton>
          </ul>
...
export function SidebarButton({
  ...
  return (
    <li
      ...
      } md:hover:cursor-pointer md:hover:bg-slate-300 md:hover:text-slate-900 md:hover:drop-shadow-md `}
    >
      <Link className={"flex items-center"} href={href}>
        ...
        <div
          className={`mr-8 h-6 w-6 transition-colors ${
            isPath
              ? "text-slate-900"
              : "text-slate-500 group-hover:md:text-slate-900"
          }  `}
        >
          {icon}
        </div>
  ...
  return (
    <li
      className={`font-bold p-2 rounded-md group transition-colors bg-rose-200 text-rose-500 
            md:hover:cursor-pointer md:hover:bg-rose-300 md:hover:text-rose-900 md:hover:drop-shadow-md `}
    >
      <button className={"flex items-center w-full"} onClick={handleOnClick}>
        <div className={"flex-grow text-center"}>{children}</div>
        <div
          className={`mr-8 h-6 w-6 transition-colors text-rose-500 group-hover:md:text-rose-900  `}
        >
          {icon}
        </div>
      </button>
    ...
    <li
      className={` ${
        activeTemplate?.finish
          ? "text-rose-900 font-bold p-2 bg-rose-300 rounded-md group transition-colors md:hover:bg-rose-400"
          : "text-emerald-900 font-bold p-2 bg-emerald-300 rounded-md group transition-colors md:hover:bg-emerald-400"
      } md:hover:cursor-pointer `}
    >
      <button
        className={"flex w-full items-center"}
        onClick={toggleMarkedButton}
      >
...
function DeleteTemplate() {
  ...
  async function deleteTemplateHandler() {
    const { user, id: templateId } = router.query as {
      user: string;
      id: string;
    };
  ...
  return (
    <li
      className={`text-rose-900 font-bold p-2 bg-rose-300 rounded-md group transition-colors md:hover:cursor-pointer md:hover:bg-rose-400 `}
    >
      <button
